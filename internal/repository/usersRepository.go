...
func (s *UserRepository) Create(user *models.User) (int64, error) {
	result, err := s.DB.Exec(`INSERT INTO users (name, username, password, is_admin) VALUES (?, ?, ?, 0)`, user.Name, user.Username, user.Password)
	if err != nil {
		return 0, err
	}
...
func (s *UserRepository) GetByID(id int) (*models.User, error) {
	row := s.DB.QueryRow(`SELECT id, name, username, password, is_admin FROM users WHERE id = ?`, id)
...
func (s *UserRepository) GetByUsername(username string) (*models.User, error) {
	row := s.DB.QueryRow(`SELECT id, name, username, password, is_admin FROM users WHERE username = ?`, username)
